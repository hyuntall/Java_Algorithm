...
public class Solution {
    public long solution(int cap, int n, int[] deliveries, int[] pickups) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < deliveries[i]; j++) dStack.push(i + 1);
            for (int j = 0; j < pickups[i]; j++)    pStack.push(i + 1);
        }
        ...
        while (!dStack.isEmpty()) {
            ...
            for (int i = 0; i < cap; i++) if (!dStack.isEmpty()) dStack.pop();
            answer += last * 2L;
        }
        while (!pStack.isEmpty()) {
            int last = pStack.peek();
            for (int i = 0; i < cap; i++) if (!pStack.isEmpty()) pStack.pop();
            answer += last * 2L;
        }
        return answer;
    }
